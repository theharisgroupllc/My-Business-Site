...
const navLinkClass =
  "rounded-md px-2.5 py-1.5 text-slate-700 transition hover:bg-white/80 hover:shadow-md hover:underline hover:decoration-brand-teal hover:decoration-2 hover:underline-offset-4 focus-visible:bg-white/80 focus-visible:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-teal/30 active:shadow-md";
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-col items-stretch gap-3 px-4 py-4 sm:flex-row sm:flex-wrap sm:items-center md:px-6">
        ...
        <nav className="flex w-full min-w-0 flex-wrap items-center gap-x-4 gap-y-2 pl-2 text-sm font-medium sm:flex-1 sm:pl-4 md:w-auto md:flex-none md:justify-center md:gap-x-5 md:pl-8 lg:absolute lg:left-1/2 lg:w-auto lg:-translate-x-1/2 lg:gap-x-7 lg:pl-10">
          ...
          <div ref={shopRef} className="relative">
            <button
              ...
              className={`${navLinkClass} inline-flex cursor-pointer items-center gap-1 border-0 bg-transparent font-medium`}
            >
              Shop
              <svg className={`h-4 w-4 transition ${shopOpen ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} aria-hidden>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {shopOpen && (
              <div className="absolute left-0 z-50 mt-3 grid w-[min(20rem,calc(100vw-2rem))] grid-cols-1 gap-2 rounded-lg border border-slate-200 bg-white p-3 shadow-lg sm:grid-cols-2">
          ...
          <Link href="/track-order/" className={navLinkClass}>
            Track Order
          </Link>
        </nav>
